...
import { FiArrowLeft, FiSave, FiEye, FiUpload, FiLink2, FiExternalLink } from 'react-icons/fi'
    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="font-grotesk text-3xl font-black text-slate-100">New Post</h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => handleSave(false)} disabled={saving} className="btn-secondary py-2 text-sm">
            <FiSave /> Save Draft
          </button>
          <button onClick={() => handleSave(true)} disabled={saving} className="btn-primary py-2 text-sm">
            {saving ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <FiEye />}
            Publish
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        <button
          onClick={() => setBlogType('written')}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border transition-all ${blogType === 'written' ? 'bg-violet-500/20 border-violet-500/40 text-violet-300' : 'border-slate-700/50 text-slate-400 hover:text-slate-200 hover:border-slate-600'}`}
        >
          <MdOutlineCode /> Written Blog
        </button>
        <button
          onClick={() => setBlogType('embed')}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border transition-all ${blogType === 'embed' ? 'bg-violet-500/20 border-violet-500/40 text-violet-300' : 'border-slate-700/50 text-slate-400 hover:text-slate-200 hover:border-slate-600'}`}
        >
          <FiLink2 /> External Link
        </button>
      </div>

      <div className="grid lg:grid-cols-3 gap-6">
        {/* Main: Editor or Embed */}
        ...
        <div className="space-y-4">
          <div className="glass border-glow rounded-2xl p-5 space-y-4">
            ...
            <div>
              <label className="block text-xs text-slate-500 uppercase tracking-wider mb-2">Read Time (minutes)</label>
          ...
          <div className="glass border-glow rounded-2xl p-5 space-y-3">
            ...
            <div
              ...
              style={{ background: coverImage ? undefined : coverColor }}
            >
              {coverImage
          ...
          {/* Cover Color (shown when no photo) */}
          {!coverImage && (
